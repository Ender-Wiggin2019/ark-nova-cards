...
function filterFunction(id) {
  ...
  if (btnElements.length > 0) {
    for (i = 0; i < x.length; i++) {
      show = false;
      for (j = 0; j < btnElements.length; j++) {
        if (x[i].className.indexOf(btnElements[j].id) > -1) {
          show = true;
        }
        if (show == true) {w3AddClass(x[i], "show");}
        else {w3RemoveClass(x[i], "show"); }
  ...
  //filter by Enclosure
  if (btnEnclosure.length > 0) {
    for (i = 0; i < x.length; i++) {
      show = false;
      for (j = 0; j < btnEnclosure.length; j++) {
        if (x[i].className.indexOf(btnEnclosure[j].id) > -1) {
          show = true;
        }
        if (show == true) {w3AddClass(x[i], "show");}
        else {w3RemoveClass(x[i], "show"); }
...
function selectCard (clickedCard) {
  ...
  if ($('#buttonsContainer:visible').length > 0) {
    ...
    if (selectedCards.length > 0) {
      document.getElementById("btn-selectedCards").href = "http://arknova.ender.lol/arknova-cards-list.html" + selectedCards;
